...
import { PreviewCommandOptionsStruct } from './types.mjs';
...
`;
const buildDeployable = async ({ argv, targetPath }) => {
    if (argv.basePath) {
        log.warn(chalk `The {yellow basePath} flag is depcrecated. The preview now deploys to a relative root by default. This flag is no longer needed`);
    }
...
const getConfig = async ({ argv, targetPath }) => {
    ...
    log.debug(`Target Path: ${targetPath}`, normalizePath(targetPath));
    newline();
...
const prepareBuild = async ({ targetPath, argv }) => {
    const buildPath = await getTempPath('preview');
    const { exclude } = argv;
...
const start = async ({ targetPath, argv }) => {
    const common = { argv, targetPath };
