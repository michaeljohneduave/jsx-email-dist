...
export const normalizePath = (filename) => filename.split(win32.sep).join(posix.sep);
export const getTempPath = async (type) => {
...
export const build = async (options) => {
    ...
    // proper named export
    const componentExport = template.Template;
    if (typeof componentExport !== 'function') {
        log.error(chalk `{red Template Export Problem:} ${path} doesn't have an export  of a JSX Element named \`Template\`. If you feel this is a bug, please open a new issue.`);
        process.exit(1);
    }
    ...
    if (plain) {
        ...
        if (writeToFile)
            await writeFile(`${writePath}.txt`, plainText, 'utf8');
        if (!html)
            return {
                compiledPath,
                html: null,
    ...
    const htmlText = await render(component, argv);
    if (writeToFile)
        await writeFile(`${writePath}.html`, htmlText, 'utf8');
    return {
        compiledPath,
        html: htmlText,
...
export const buildTemplates = async ({ targetPath, buildOptions }) => {
    ...
    if (exclude)
        targetFiles = targetFiles.filter((path) => !micromatch.isMatch(path, exclude));
    if (!silent) {
    ...
    const result = await Promise.all(compiledFiles.map(async ({ entryPoint, path }, index) => {
...
export const command = async (argv, input) => {
    if (input.length < 1)
        return false;
